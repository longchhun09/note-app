...
const userInitials = computed(() => {
  ...
  if (username.includes(' ')) {
    // If name has spaces (first and last name)
    const names = username.split(' ');
    return (names[0][0] + names[names.length - 1][0]).toUpperCase();
  } else {
    // Otherwise just use first 1-2 characters
...
<template>
  <div class="bg-white w-full min-h-screen flex flex-col">
    <nav class="bg-white border-b border-gray-200 shadow-sm transition-all duration-300">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          <!-- Logo/Brand -->
          <div class="flex items-center flex-shrink-0">
            <router-link to="/" class="text-xl font-bold text-blue-600 hover:text-blue-800 transition-colors duration-200">
              Note App
            </router-link>
          </div>
                ...
                <div class="ml-3 relative user-dropdown">
                  <div>
                    <button @click="isDropdownOpen = !isDropdownOpen" 
                      ...
                      <div class="h-9 w-9 rounded-full bg-blue-600 flex items-center justify-center text-white font-medium shadow group-hover:bg-blue-700 transition-colors duration-200">
                        {{ userInitials }}
                      </div>
                      <!-- Username (Desktop only) -->
                      <span v-if="authStore.user" class="ml-2 text-gray-700 font-medium hidden lg:block group-hover:text-gray-900">
          ...
          <div class="flex items-center sm:hidden">
            <button v-if="isAuthenticate" 
              ...
              <svg v-if="!isMobileMenuOpen" class="block h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <div v-if="isAuthenticate && isMobileMenuOpen" class="sm:hidden overflow-hidden">
          ...
          <div class="pt-4 pb-3 border-t border-gray-200">
            <div class="flex items-center px-4">
              ...
              <div class="ml-3">
                <div v-if="authStore.user" class="text-base font-medium text-gray-800">{{ authStore.user.username }}</div>
              </div>
            </div>
